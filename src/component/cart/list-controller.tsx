...
export default function CartListController(){
    ...
    const RemoveCartList = () =>{
        ...
        const result = parse.filter((list:any)=>!check.includes(list.product))
            ...
            <label className={styles['all-check']}>
                <label className={public_style.checkbox}>
                    <input type={"checkbox"}
                           ...
                           onChange={(e)=>dispatch(allCheck({checked:e.target.checked,list:data.map((item:any)=>item.product_id)}))}/>
                    <span>
